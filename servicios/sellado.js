...
module.exports = (function() {
  ...
  //Leyendo datos de la tabla Sellado
  router.get('', function(req, res) {
    if (paginate) {
      ...
      var read_query = "SELECT sellado.id, sellado.codhash, sellado.fecha, sellado.tipo, usuario.usuario FROM " + ('sellado') +','+ ('usuario') + ' WHERE sellado.idU = usuario.id'+ ' LIMIT ' + page_limit + " OFFSET " + offset;
    } else {
      var read_query = "SELECT sellado.id, sellado.codhash, sellado.fecha, sellado.tipo, usuario.usuario FROM " + ('sellado') +','+ ('usuario') + ' WHERE sellado.idU = usuario.id';
    }
  ...
  //Insertando datos en la tabla Sellado
  router.post('', function(req, res) {
    ...
    sequelize.query("INSERT INTO `" + ('sellado') + "` (" + keys + ") VALUES (" + values + ")", {type: sequelize.QueryTypes.INSERT}).then(function(id) {
      res.status(201);
